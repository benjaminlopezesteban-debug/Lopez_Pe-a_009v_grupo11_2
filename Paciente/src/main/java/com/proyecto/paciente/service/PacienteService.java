...
import com.proyecto.paciente.exception.BadRequestException;
import com.proyecto.paciente.exception.ResourceNotFoundException;
...
public class PacienteService {
    ...
    public PacienteFeignResponse findByRut(String rut) {
        ...
        if (p == null) throw new ResourceNotFoundException("Paciente no encontrado con rut: " + rut);
        return toFeign(p);
    }

    @Transactional(readOnly = true)
    public List<PacienteFeignResponse> searchByNombre(String pnombre, String snombre, String appaterno) {
        log.info("event=search_paciente pnombre={} snombre={} appaterno={}", pnombre, snombre, appaterno);
        // snombre se ignora en este repositorio básico; se puede ampliar a contains en ambos
