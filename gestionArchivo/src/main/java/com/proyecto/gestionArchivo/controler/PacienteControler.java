...
public class PacienteControler {
    ...
    private final PacienteService pacienteService;                  //vuelve la inyeccion de constructores
    ...
    @GetMapping("/{id}")
    public ResponseEntity<PacienteModel> findById(@PathVariable Long id){//Programación Funcional, Evita el bucle de if y evita crear el trycath esta optimizado 
        Optional<PacienteModel> paciente = pacienteService.findById(id);
        return paciente.map(ResponseEntity::ok) //Si encuentra responde con un ok codigo 200
                .orElse(ResponseEntity.notFound().build()); //Entrega la respuesta si esta vacio. Responde not found con un cuerpo vacio 404
    }
    ...
    @PostMapping
    public ResponseEntity<PacienteModel> save(@RequestBody PacienteModel paciente) {
